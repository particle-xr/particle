...
namespace particle::vulkan::su {
const uint64_t FenceTimeout = 100000000;

template<typename Func>
void
oneTimeSubmit(const vk::CommandBuffer& commandBuffer,
              ...
              const Func& func)
{
  ...
  func(commandBuffer);
  commandBuffer.end();
  queue.submit(vk::SubmitInfo{ .waitSemaphoreCount = 0,
...
template<typename Func>
void
oneTimeSubmit(const vk::Device& device,
...
template<class T>
void
copyToDevice(const vk::Device& device,
...
template<class T>
VULKAN_HPP_INLINE constexpr const T&
...
void
setImageLayout(const vk::CommandBuffer& commandBuffer,
               vk::Image image,
               vk::Format format,
...
WindowData
createWindow(const std::string& windowName, const vk::Extent2D& extent);
...
struct BufferData
{
  ...
  template<typename DataType>
  void upload(vk::Device const& device,
    ...
    oneTimeSubmit(
      device, commandPool, queue, [&](vk::CommandBuffer const& commandBuffer) {
        commandBuffer.copyBuffer(
          ...
          buffer,
          vk::BufferCopy{ .srcOffset = 0, .dstOffset = 0, .size = dataSize });
      });
...
struct SurfaceData
{
  SurfaceData(vk::Instance const& instance,
...
public:
  CheckerboardImageGenerator(
    std::array<uint8_t, 3> const& rgb0 = { { 0, 0, 0 } },
...
{
public:
  PixelsImageGenerator(vk::Extent2D const& extent,
                       size_t channels,
...
struct TextureData
{
                ...
                ImageGenerator const& imageGenerator)
  {
    void* data =
      needsStaging
        ? device.mapMemory(
            stagingBufferData->deviceMemory,
            0,
    ...
    if (needsStaging) {
      ...
      vk::BufferImageCopy copyRegion{
        ...
        .imageExtent = vk::Extent3D{ .width = extent.width,
                                     .height = extent.height,
                                     .depth = 1 }
      };
      commandBuffer.copyBufferToImage(stagingBufferData->buffer,
                                      imageData->image,
                                      vk::ImageLayout::eTransferDstOptimal,
                                      copyRegion);
...
VULKAN_HPP_INLINE TargetType
checked_cast(SourceType value)
{
...
bool
contains(std::vector<vk::ExtensionProperties> const& extensionProperties,
         std::string const& extensionName);

vk::CommandPool
createCommandPool(vk::Device const& device, uint32_t queueFamilyIndex);

vk::DebugUtilsMessengerEXT
createDebugUtilsMessengerEXT(vk::Instance const& instance);
...
createDescriptorSetLayout(
  vk::Device const& device,
  std::vector<
    std::tuple<vk::DescriptorType, uint32_t, vk::ShaderStageFlags>> const&
    bindingData,
...
vk::Instance
createInstance(std::string const& appName,
...
vk::Format
pickDepthFormat(vk::PhysicalDevice const& physicalDevice);
...
void
submitAndWait(vk::Device const& device,
...
updateDescriptorSets(
  ...
  std::vector<std::tuple<vk::DescriptorType,
                         vk::Buffer const&,
                         vk::BufferView const&>> const& bufferData,
  std::vector<TextureData> const& textureData,
  uint32_t bindingOffset = 0);
}
